...
if __name__ == "__main__":
    ...
    #
    # 4) Convert to datetime for easy offset checks
    df_pred_aug["Prediction_Month"] = pd.to_datetime(df_pred_aug["Prediction_Month"], format="%Y-%m")
    df_pred_aug["Actual_Month"] = pd.to_datetime(df_pred_aug["Actual_Month"], format="%Y-%m")
    # 4) Show sample for one SKU, e.g. SKU_1
    sku_filter = "SKU_1"
    df_sku1 = df_pred_aug[df_pred_aug["SKU"] == sku_filter].copy()
    #
    # If you do that, then print or save to CSV:
    df_pred_aug.to_csv("sample_data.csv", index=False)