...
def compute_residual_counts(actuals: np.ndarray, preds: np.ndarray) -> tuple[int, int]:
    """
...
for (sku, actual_month), grp in df.groupby(['SKU', 'Actual_Month']):
    ...
    pos_count, neg_count = compute_residual_counts(y_true, y_pred)
    ...
    metrics = {
        ...
        'Residual_Pos_Count': pos_count,
        'Residual_Neg_Count': neg_count,
